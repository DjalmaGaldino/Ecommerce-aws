  ...
  event.Records.forEach((record)=> {
    if(record.eventName === 'INSERT') {
      ...
      if(record.dynamodb!.OldImage!.pk.S === '#transaction') {
        console.log('Invoice trasacntion event received')
        promises.push(processExpiredTrasanction(record.dynamodb!.OldImage!))
      }
...
async function processExpiredTrasanction(invoiceTransactionImage: {[key: string]: AttributeValue}): Promise<void> {
  ...
  if(invoiceTransactionImage.transactionStatus.S === 'INVOICE_PROCESSED') {
    ...
    const sendStatusPromise = invoiceWSService.sendInvoiceStatus(transactionId, connectionId, 'TMEOUT')
...
async function createEvent(invoiceImage: {[key: string]: AttributeValue}, eventType: string): Promise<void> {
  ...
  await ddbClient.put({
    TableName: eventsDdb,
    Item: {
      pk: `#invoice_${invoiceImage.sk.S}`, // #ivoice_ABC-123
