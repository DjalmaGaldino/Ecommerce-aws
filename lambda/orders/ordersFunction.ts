...
const cognitoIdentityServicerProvider = new CognitoIdentityServiceProvider()

const orderRepository = new OrderRepository(ddbClient, ordersDdb)
const productRepository = new ProductRepository(ddbClient, productsDdb)
const authInfoService = new AuthInfoService(cognitoIdentityServicerProvider)
...
export async function handler(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
  ...
  const authenticateUser = await authInfoService.getUserInfo(event.requestContext.authorizer)
  ...
  if(method === 'GET') {
    if(event.queryStringParameters) {
      ...
      if(isAdmin || email === authenticateUser) {  
        if (email) {
          if(orderId) {
    ...
    if (!isAdmin) {
      orderRequest.email = authenticateUser
    ...
    if (isAdmin || email === authenticateUser) {
      try {
        ...
        const eventResult = await sendOrderEvent(orderDelete, OrderEventType.CREATED, lambdaRequestId)
        console.log(
          `Order created event sent - OrderId: ${orderDelete.sk}
